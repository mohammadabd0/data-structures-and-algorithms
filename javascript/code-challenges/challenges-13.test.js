...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(number => `${number.substring(1, 4)}${number.substring(6, number.length).split('-').join('')}`);
...
const allHappy = (arr) => {
  // Solution code here...
  let newArray = false;
  arr.length === 0 ? newArray = true : newArray = arr.reduce((acc, Value) => Value.includes(':)') === false ? acc = false : acc , true);
  return newArray;
};
