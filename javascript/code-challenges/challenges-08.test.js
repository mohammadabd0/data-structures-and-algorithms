...
const sortByChildren = (charArray) => {
  // Solution code here...
  return charArray.sort((first, sec) => first.children.length - sec.children.length);
...
const isCapitalized = (str) => {
  ...
  if (str.match(Capitalized)) {
    newArray = str.match(Capitalized);
  }
  return newArray;
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let Capitalized = /^\b[A-J]\w*/g;
    let newArray = [];
    arr.forEach(idx => {
        if (idx.match(Capitalized)) {
          newArray.push(idx);
        }
    });
    return newArray;
